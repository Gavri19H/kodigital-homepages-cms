...
// Asset identity is sha256 + bytes + fileName — all three are pure functions of the built
// bytes, so they are byte-stable on every toolchain. gzipBytes deliberately is NOT pinned:
// a gzip length is a property of the compressor, not of the asset. Identical bytes compress
// to different lengths on different zlib builds (Node 22 ships zlib 1.3.1 and measures the
// conversions bundle at 41831; Node 25 ships zlib-ng and measures the same sha256 at 41759),
// so pinning a measurement here makes the manifest reproducible on exactly one Node version
// and permanently stale everywhere else. The live per-platform measurement is still taken on
// every build, still gated against the gzip budgets in buildProduct(), and still printed by
// productSummary(); only the non-reproducible pin is gone.
  ...
  for (const product of [built.conversions, built.reporting]) {
    ...
    for (const entry of actualNames) {
      ...
      if (active !== undefined) {
        ...
        // Staleness is decided by identity only (full byte equality, plus byte length and
        // sha256 as explicit restatements of it). gzipBytes is excluded on purpose: it is a
        // compressor artifact, so comparing it can only ever reject a byte-identical asset
        // because the local zlib differs from the one that built the committed copy.
        if (
