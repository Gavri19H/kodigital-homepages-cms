...
//     answer-field universe. SAME derivation ui-quotes.ts's own routingData
...
//
// renderRoutingRulesPanel/ROUTING_RULES_SCRIPT (above) stay BYTE-IDENTICAL —
// they remain the quote/variant editor's own condition-envelope editor (still
// bundled there via ui-quotes.ts) and are NOT reused here: that script's
// persistence model writes HOST-ROW hidden carriers for the surrounding page's
// own big variant-PUT Save button, which does not exist on this per-auction
// page. This is a NEW, self-contained, REST-driven editor — reusing the SAME
// §21.4 condition builder (window.lgRulesBuilder / RULES_BUILDER_SCRIPT) the
// quote-rules rail (renderQuoteRulesRail, above) mounts, the identical pattern.
//
