// Listicles ClickHouse HTTP client (design contract §17/§18).
//
// A fresh, neutrally-named, fetch-based client for the CH Cloud HTTP
// interface — PORTED in pattern from the dashboard's lib/clickhouse.ts but
// written from scratch here (no cross-repo import; zero shared identifiers).
//
// Auth: the three worker SECRETS CH_URL / CH_USER / CH_PASSWORD (never in
// wrangler.toml — see infra/listicles/clickhouse-apply.md). We send them as
// the X-ClickHouse-User / X-ClickHouse-Key headers (the same shape the apply
// doc's curl uses), so the worker and the operator authenticate identically.
//
// Fail-open by design (like the Firehose path + the Phase-7 stream): when any
// of the three secrets is absent the client is UNCONFIGURED — `query` makes NO
// HTTP request and returns an empty, structured result. A configured query
// that fails (HTTP error / timeout / parse) throws a typed ListicleChError so
// the caller's own try/catch can isolate it (mirror-sync isolates per table;
// reconciliation is fail-open per §31.6).

import type { Env } from "../env";

export class ListicleChError extends Error {
  readonly statusCode: number;
  constructor(message: string, statusCode: number) {
    super(message.length > 500 ? message.slice(0, 500) : message);
    this.name = "ListicleChError";
    this.statusCode = statusCode;
  }
}

export interface ListicleChQueryResult<T> {
  rows: T[];
...
export interface ListicleChClient {
   ...
   * parsed JSONEachRow rows. No-op empty result when unconfigured; throws
...
export function createListicleChClient(
  env: Partial<Env>,
  ...
  return {
    configured,
    async query<T>(sql: string, params?: Record<string, string | number>): Promise<ListicleChQueryResult<T>> {
      if (!configured) {
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          throw new ListicleChError(body || `ClickHouse HTTP ${resp.status}`, resp.status);
        }
