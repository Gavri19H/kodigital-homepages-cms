...
test.describe("P3b Funnel-builder board — live journeys (§8.2)", () => {
  ...
  // R2 SRC-11B (owner: "the themes and the templates are moving to the top
  // bar, why you kept the old and wrong option in the funnel builder??").
  // The column Template pickchip no longer opens an embedded apply-popover —
  // it NAVIGATES to the top-bar Templates tab, exactly like its Theme sibling
  // (the "A/B badge navigates to the A/B tab" test just above is the SAME
  // pattern). Superseded test: the OLD embedded-popover mechanism
  // (data-template-menu / apply-in-builder) this test drove is REMOVED by
  // the same owner ruling — retired, not merely renamed. Runs on both engines.
  test("template pickchip navigates to the top-bar Templates tab (no embedded popover)", async ({ page }) => {
    const quote = await seedQuote(apiCtx);
    await openEditor(page, quote.public_id);
    ...
    // [data-template-menu] is a SHARED generic popover container (openPopoverList,
    // still used by the +section pickers) — present in the DOM but must stay
    // hidden; it must NEVER open for the Template chip anymore.
    await expect(page.locator("[data-template-menu]")).not.toBeVisible();
  });
