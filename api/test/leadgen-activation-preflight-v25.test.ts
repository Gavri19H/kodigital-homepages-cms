...
async function seedQuote(
  ...
  h.sdb.prepare("INSERT INTO leadgen_funnel_pages (public_id, quote_id, position, name) VALUES (?, ?, 0, NULL)").run(sharedPagePublicId, quoteRow.id);
  h.sdb
    .prepare(
...
describeDb("bumpActiveVariantContentVersions (03 §3.1)", () => {
  it("bumps content_version on every ACTIVE variant of the funnel and ONLY those", async () => {
    ...
    h.sdb
      .prepare(
        "INSERT INTO leadgen_funnel_variants (public_id, funnel_id, variant_label, status) VALUES (?, ?, 'B', 'active')",
      )
      .run(mintPublicId("funnel_variant"), seeded.funnelId);
    h.sdb
      .prepare(
        "INSERT INTO leadgen_funnel_variants (public_id, funnel_id, variant_label, status) VALUES (?, ?, 'C', 'archived')",
      )
      .run(mintPublicId("funnel_variant"), seeded.funnelId);
