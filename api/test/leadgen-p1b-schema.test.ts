...
// ---------------------------------------------------------------------------
// R4-34 — MQG save trap: orphan shared choices pruned in place
// ---------------------------------------------------------------------------
function mqg(shared: { label: string; value: string; analytics_id: string }[], rows: Record<string, unknown>[]): LeadgenComponentNode {
  return { type: "MultiQuestionGrid", question_id: "q_grid", choices: shared, props: { rows } };
}
const CH = (v: string) => ({ label: v.toUpperCase(), value: v, analytics_id: v });

describe("Round-4 R4-34 (P1b) — MQG orphan-shared-choice prune kills the save trap", () => {
  it("a 6-shared-choice grid with 2 rows SAVES (ok), orphans pruned to <=4, no pill-set jargon", () => {
    const node = mqg(
      [CH("yes"), CH("no"), CH("x3"), CH("x4"), CH("x5"), CH("x6")],
      [
        { label: "R1", internal_field: "r1", default: "yes" },
        { label: "R2", internal_field: "r2", default: "no" },
      ],
    );
    const c = content(node);
    const r = validateSectionContent(c);
    expect(r.ok).toBe(true);
    expect((node.choices as unknown[]).length).toBeLessThanOrEqual(4);
    expect(r.errors.some((e) => /must number|pill set/.test(e.message))).toBe(false);
  });

  it("keeps a referenced default during the prune (drops only unreferenced excess)", () => {
    const node = mqg(
      [CH("yes"), CH("no"), CH("x3"), CH("x4"), CH("x5"), CH("x6")],
      [
        { label: "R1", internal_field: "r1", default: "x6" }, // references the LAST shared pill
        { label: "R2", internal_field: "r2", default: "yes" },
      ],
    );
    const c = content(node);
    expect(validateSectionContent(c).ok).toBe(true);
    const values = (node.choices as { value: string }[]).map((x) => x.value);
    expect(values).toContain("x6"); // referenced → survived
    expect(values).toContain("yes"); // referenced → survived
    expect(values).not.toContain("x5"); // unreferenced excess → pruned
    expect(values.length).toBeLessThanOrEqual(4);
  });

  it("a valid grid (<= MAX shared) is UNTOUCHED — the prune is a no-op", () => {
    const node = mqg(
      [CH("yes"), CH("no")],
      [{ label: "R1", internal_field: "r1", default: "yes" }],
    );
    const c = content(node);
    validateSectionContent(c);
    expect((node.choices as unknown[]).length).toBe(2);
  });

  it("a still-too-FEW shared set fails with the PLAIN-language message (no raw jargon)", () => {
    const node = mqg([CH("only")], [{ label: "R", internal_field: "r", default: "only" }]);
    const r = validateSectionContent(content(node));
    expect(r.ok).toBe(false);
    const msg = r.errors.find((e) => e.path.endsWith(".choices"))?.message ?? "";
    expect(msg).toContain("A question grid's shared answers must be 2-4");
    expect(msg).not.toContain("pill set");
  });
});
