...
| 8 | Frame hint | 6.3 | 1 | **PROVEN**: gate1-parity ("the funnel-layout toggle renders and the skeleton ships VISIBLE by default") + gate2-strings (funnel-layout copy). Mechanism note (not a divergence): default-ON is SSR-always-visible + client `hidden`-on-toggle-off, not a client-state-default like the golden's demo — documented in-file. **U15 erratum (2026-07-15)**: the visible label is "Show funnel layout" / "Funnel layout" (golden "Frame hint"/"Funnel frame" renamed for operator clarity; renderFrameHintSkeleton reclassified golden:false). |
