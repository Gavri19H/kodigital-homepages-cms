...
| U13 | Canvas drag dead in real Chrome — SECOND retest failure (2026-07-15, post-#117) | Operator: "no element can be dragged to a new custom location" in real Chrome; selection (click) works | **ROOT-CAUSED (mechanical probe, 2026-07-15):** Chrome does not deliver held-button mousemove streams to listeners inside a `sandbox="allow-same-origin"` (scripts-disabled) srcdoc iframe. Minimal 3-frame probe (scratchpad sandbox-mouse-repro.mjs): frame A = the canvas's exact sandbox → drag-move protocol call TIMEOUT, stream truncated (mousedown/mouseup/click DO deliver — matching the operator's working selection); frame B (`+allow-scripts`) and frame C (no sandbox) deliver fully; firefox delivers in all three — why every firefox gesture gate passed while the operator's Chrome failed twice. REFUTES R0a's "CDP/automation environment limitation" classification AND U11's "failure class removed" claim — the class was never native-DnD-specific; BOTH move mechanisms die inside the frame. Shield probe (shield-repro.mjs): a parent-doc overlay raised ON the in-frame mousedown receives NOTHING in either engine (chromium still stalls; firefox implicitly captures the stream to the originating document) — in-frame gesture streams cannot be rescued from outside; the frame itself must deliver. **FIX (probe-proven both engines, csp-sandbox-repro.mjs):** `sandbox="allow-same-origin allow-scripts"` + `<meta http-equiv="Content-Security-Policy" content="script-src 'none'; object-src 'none'; base-uri 'none'">` as the first content element of the srcdoc head (only conductor-emitted bytes precede it) — full event delivery (chromium drag ok, mousemove 8/8) while every script vector stays dead (inline script, on* attributes, javascript: URL: all proven non-executing; sandbox still denies forms/popups/top-navigation). PERMANENT COUNTERMEASURE: the gesture lane runs on BOTH engines (chromium + firefox) — fail-before = the documented chromium-attempt hang at HEAD; the firefox-only blind spot class dies. CLOSURE: both-engine gesture gates green + operator drag confirmation in real Chrome. **FIXED + adversarial review SHIP (2026-07-15, fix cycle 3): sandbox+CSP landed; chromium gesture lane ARMED (U13 specs 6/6 chromium + 6/6 firefox, real page.mouse; fail-before recorded: mouse.move step 1/5 8000ms HANG at HEAD); canvas script-inertness security gate green (5 hostile vectors inert, CSP-first proven, reviewer-audited escape paths).** | OPEN — pending operator Chrome drag confirmation |
| U14 | Continue button uncenterable + left-aligned (canvas AND live) | Operator: "can't centerize the 'continue' button in any way"; renders left-hugged inside the question card | **ROOT-CAUSED (measured, 2026-07-15):** designs/default-funnel/styles.ts gives `.lg-continue` `margin-left/right:auto`, but the `.lg-btn` base sets `display:inline-flex` — auto margins compute to 0 on inline-level boxes. Live-render probe (u14-continue-probe.mjs, real studio canvas): computed width 320px / maxWidth 320px / marginLeft 0px / marginRight 0px; button x=387 where card-centered = 480 (600 column, 46px card padding, 320 pill). Affects LIVE funnels identically (same stylesheet + shared renderContinueButton). The U12 gates asserted the css TEXT (margin:auto present in the rule) and no gate ever measured the button's rendered x — the exact self-certifying-artifact class §A warns about. Per contract §14.6 ("full-width centred pill", max-width 320 token) + golden :350 (width:100% of the column): the centered pill IS the design — no new control is owed (§8.5b read-only stands); the RENDER is wrong. FIX: block-level flex on `.lg-continue` (label centering via the existing btnBase flex alignment); AutoAdvanceButton audited against the golden for the same class of defect; NEW GATE: measured |button-center − column-center| ≤ 1px in the studio canvas AND the live /lg serve, on chromium + firefox. **FIXED + adversarial review SHIP (2026-07-15, fix cycle 3): display:flex landed; canvas centering measured ≤1px both engines; LIVE /lg centering browser-measured diff=0.00px (btn center 640 = card content center 640); AutoAdvance audited — golden silent, left unchanged.** | OPEN — pending operator composition retest |
| U15 | "Frame" jargon in product copy | Operator: "what is 'frame'??? this is totally unclear!" | Product copy leans on an internal noun the operator never adopted: canvas pill "FUNNEL FRAME" (golden:122 verbatim), name-tag chip "from frame" (golden:354 / studio:5663), inspector eyebrow "Inherited from the frame" + notice "Color, size & position come from the funnel frame…" + 3× "Change in frame →" (studio:2220–2231), toolbar "Frame hint" (studio:1389), scope pill "Funnel frame" (studio:1214), frame-scope strip notices. The golden itself carries the term, so this is a DELIBERATE post-golden divergence ordered by the operator (same protocol as the s=300/l=480 errata): every user-facing string switches to self-explanatory copy that names the destination — concept: "funnel layout — shared by every section in this funnel"; actions: "Edit in Quote Builder →"; hint toggle names what it shows ("Show funnel layout"); the inspector notice explains ownership in one sentence. Golden-region allowlist entries reclassified `golden:false` with an erratum row for operator sign-off. **FIXED + adversarial review SHIP (2026-07-15, fix cycle 3): full sweep across Section Builder AND Quote Builder surfaces (review MAJOR-1 caught the Quote-Builder residue — page frame/frame settings/frame template/Funnel frame preview/Page frame scope label — all renamed); coordinated 3-file rename (Move to funnel layout + activation message + schema message); PERMANENT GATE test/leadgen-u15-jargon-copy-pin.test.ts scans the RENDERED studio+island+quote-builder route for 7 banned phrases (caught PROBLEM_SCOPE_LABELS on first run — real fail-before); allowlist split: renderStudioInspector restored golden:true, only the extracted renderStyleContinueBlock carries golden:false (census 34/22/12/0/0). ERRATUM for operator sign-off: copy diverges from golden verbatim strings by operator order.** | OPEN — pending operator sign-off of the new copy |
...
**FIX CYCLE 3 (2026-07-15, post-#117 SECOND operator retest failure → rows U13/U14/U15): SHIP.**
Adversarial review round 1 = FIX-FIRST (MAJOR-1 Quote-Builder "frame" residue the new copy
actively routes the operator into + 3 MINORs: live centering not browser-measured, allowlist
granularity, stale hang-doctrine comment) → all fixed in-phase → re-review **SHIP, stated
unambiguously**. Conductor-verified gates at branch HEAD 8045758: tsc 0 · vitest 5,302/5,302
(374 files) · Playwright 310/310 (3 shards, 250 chromium + 60 firefox) · verify:all exit 0
(runtime bundle byte-identical 40,908 · jargon 0 · golden census 34 blocks = 22 golden:true /
12 golden:false / 0 unclassified / 0 stale) · U13 both-engine gesture gate 12/12 · live /lg
centering measured diff=0.00px. NEW PERMANENT GATE CLASSES: chromium gesture lane (the
operator's own engine now drives real drags — the firefox-only blind spot is dead), canvas
script-inertness (sandbox allow-scripts is provably contained by first-in-head CSP script-src
'none'), U15 rendered-copy jargon pin (caught a real regression on its first run), measured
centering (canvas + live). One squash PR; number recorded in the tracker on merge. Operator
closure residue: real-Chrome drag confirmation (U13), composition retest (U14/U12), new-copy
sign-off (U15 erratum). All 9 mission-level acceptance
