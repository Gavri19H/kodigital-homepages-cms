...
| 24 | Dependencies / conditional logic | P5/P6 | `content_json.conditional` (authored+stored+validated P5) + `public/leadgen/runtime.ts` (eval P6) | dependency preview spec | PENDING — P5 authoring+storage+validation done (`conditional {when,op,value}` in the builder); RUNTIME evaluation on the funnel is P6 |
...
| 26 | Answer→Offer payload mapping | P5 | `leadgen_section_answer_maps` + `/sections/:id/validate-payload` + mapping grid | `leadgen-sections-api.test.ts` (validate-payload per-offer + B1 round-trip) | PASS — shipped early in P5 (authoring grid + derived rebuild + per-offer preview + value-map round-trip) |
